...
This is the global Claude Code feature-momentum rule file and is synced into `~/.claude/CLAUDE.md` by `agent-sync rules:install`.
...
1. **Verify Superpowers is installed globally.** Check `test -d ~/.agents/skills/using-superpowers` (or `ls ~/.claude/skills/using-superpowers` for the symlinked Claude Code copy). If missing, install it once for the whole machine — no per-worktree install needed:
...
4. **Open the worktree in a fresh editor window when a worktree was chosen.** Tell the user to open the new directory and start a new Claude Code chat there. Do not continue feature implementation in the old checkout if the new worktree was created for isolation.
